...
namespace PipesInPool.Model
{
    class Pool
    {
        ...
        public bool reulst { get; private set; }
        ...
        public double IfOverFlows()
        {
            return (Pipe1MultiplyByhours() + Pipe2MultiplyByhours());
        }


        public string PrintIfOverFlows()
        {
            if (IfOverFlows() > Volume)
            {
                return $"The Pool Overflows with {IfOverFlows()} litres";
            }
            else
            {
                return $"The pool is: {Math.Floor(PercentageFlowFromPipe1AndPipe2FromVolume())}% full. " +
                    $"Pipe 1: {Math.Floor(Pipe1FlowPercentage())}%. " +
                    $"Pipe 2: {Math.Floor(Pipe2FlowPercentage())}%";
            }
        }
    }
}
